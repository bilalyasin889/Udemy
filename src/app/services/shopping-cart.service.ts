...
export class ShoppingCartService {
  ...
  async addToCart(product: ShopProduct, productId: any){
    this.updateItem(product, productId, 1);
  }

  async removeFromCart(product: ShopProduct, productId: any){
    this.updateItem(product, productId, -1);
  }
  ...
  private async updateItem(product: ShopProduct, productId: any, quantity: number){
    let cartId = await this.getCartId();
